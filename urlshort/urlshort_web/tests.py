...
class GraphQLTestCase(TestCase):
    ...
    def test_creation_and_reuse(self):
        ...
        self.assertIn('shortUrl', resp['data'])
        key = resp['data']['shortUrl']
        resp = self.query('''query {
            shortUrl(url: "https://www.google.com/")
          }''')
        self.assertEqual(resp['data']['shortUrl'], key)
    
    def test_retrieve_full_url_by_short(self):
        resp = self.query('''query {
            shortUrl(url: "https://www.google.com/")
          }''')
        key = resp['data']['shortUrl']
        resp = self.query('query {url(shortUrl: "%s")}' % key)
